...
export const getAuthorization = () => {
    return { 'Authorization':window.localStorage.getItem('user_token')}
}
...
export const setPermissions = permissionNames => {
    permissions = {}
    for(let p in permissionNames){
        permissions[permissionNames[p]] = true
    }
}
 ...
 */
export const converListToTreeData = (data,parentId,pidField) => {
    var list =[]
    data.forEach(item=>{
        if(item[pidField] == parentId){
            item.children = converListToTreeData(data,item.id,pidField)
            list.push(item);
        }
    })
    return list;
