...
<template>
    <div :style="{width:tableWidth}" class='autoTbale'>
        <table class="table table-bordered">
            ...
            <tbody>
            <tr v-for="(item,index) in initItems" v-show="show(item) && reload" :class="{'child-tr':item.parent, 'selected': item._selected}" @click="RowClick(item, index)" :key="index">
		...
		methods: {
			...
			RowClick(data, index){
				...
				this.selectedRow = data,
					data._selected = true, this.reload = true
			},
			// 处理表头数据
			makeColumns() {
			...
			getSpaceHtml(level){
				...
				for (var i = 1; i < level; i++) {
					spaceHtml += "<i class='ms-tree-space' style='display:inline-block;width:14px;line-height:1;height:14px;'></i>"
				}
				return spaceHtml
			},
			// 数据处理 增加自定义属性监听
			...
			getChecked(){
				if(this.checkGroup.length>0){
					this.checkGroup.sort();
					var newCheckData = [this.checkGroup[0]]
					for(let i=1;i<this.checkGroup.length;i++){
						if(this.checkGroup[i]!==newCheckData[newCheckData.length-1]){
							newCheckData.push(this.checkGroup[i])
						}
					}
					return newCheckData
				}else{
